...
const SEED_USER_ID = "user_2i16HcHUzFQWrGgnAtcqx3jXn4P";
...
const generateTrasactionsForDay = (day: Date) => {
  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    SEED_TRANSACTIONS.push({
      ...
      payee: "Merchant",
      notes: "Randon transaction",
    });
  }
};

const generateTrasactions = () => {
  const days = eachDayOfInterval({ start: defaultFrom, end: defaultTo });
  days.forEach(day => generateTrasactionsForDay(day));
};
